...
+ (NSString *)ang_platformString{
    NSString *platform = [self ang_platform];
...
+ (NSUInteger)ang_getSysInfo:(uint)typeSpecifier
{
...
+ (NSUInteger)ang_cpuFrequency {
    return [self ang_getSysInfo:HW_CPU_FREQ];
}

+ (NSUInteger)ang_busFrequency {
    return [self ang_getSysInfo:HW_BUS_FREQ];
}

+ (NSUInteger)ang_ramSize {
    return [self ang_getSysInfo:HW_MEMSIZE];
}

+ (NSUInteger)ang_cpuNumber {
    return [self ang_getSysInfo:HW_NCPU];
}


+ (NSUInteger)ang_totalMemoryBytes
{
    return [self ang_getSysInfo:HW_PHYSMEM];
}
